...
<%    clases.controles.VerificarConexion();
    ...
    String[] array_aviarios= request.getParameterValues("aviario");;
    String aviario="";
  ...
  for(int i=0; i<array_aviarios.length; i++)   
    {
      ...
      PreparedStatement pt=clases.controles.connectSesion.prepareStatement("select  aviario, FORMAT(fecha,'dd-MM')fecha, muertes,caudalimetro,"
                    + "temp_min,temp_max,product,consumo_bal,product,bal_pad, product_pad  from vis_mae_ppr_grafico where aviario=("+aviario+") and "
                    + "  DATEDIFF(day,fecha,getdate())<=30 order by 2" );
        ...
        JSONArray   categories,             Dataset,            contenido_subcategorias, 
                ...
                while(rss.next()) // va a recorrer 22 veces
                  { 
                    ...
                    DataTempMin.put("backgroundColor",    "rgba(255, 0, 0, 0.5)");
                    DataTempMin.put("borderColor",  "rgba(255, 0, 0, 0.5)");
                    ...
                    DataTempMax.put("backgroundColor",    "rgba(0, 0, 255, 0.5)");
                    DataTempMax.put("borderColor",  "rgba(0, 0, 255, 0.5)");
